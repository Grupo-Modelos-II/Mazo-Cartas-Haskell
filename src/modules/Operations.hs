...
menu = do
 ...
 if option == 1 then do
  ...
  symbol <- getLine
  mallet <- pedirCarta(value, symbol)
  print mallet
  ...
  mallet <- getMallet
  print(buscarCarta value (if isJugador then mallet else getDeck))
 else do
  ...
  let isJugador = response == "Y"
  mallet <- getMallet
  print(preguntarValor symbol (if isJugador then mallet else getDeck))

 putStr "¿Repetir Proceso? (Y, N)\n"
 input <- getLine
 if input == "Y" || input == "y" then menu else putStrLn "Chauuu"